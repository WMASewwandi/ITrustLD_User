...
export const PROMO_SECTION_ID = "promotions";

function isPromotionHash(hash) {
  return hash === PROMO_SECTION_ID || hash.startsWith("promotion-");
}

export function getPromotionHashTarget() {
  if (typeof window === "undefined") return "";
  const hash = window.location.hash.replace(/^#/, "").trim();
  return isPromotionHash(hash) ? hash : "";
}

const PROMO_SCROLL_KEY = "itrust-scroll-promotion";

export function rememberPromotionScroll(targetId) {
  if (typeof window === "undefined" || !targetId) return;
  try {
    sessionStorage.setItem(PROMO_SCROLL_KEY, String(targetId));
  } catch {
    /* ignore quota / private mode */
  }
}

export function consumePromotionScrollTarget() {
  try {
    const stored = sessionStorage.getItem(PROMO_SCROLL_KEY) || "";
    sessionStorage.removeItem(PROMO_SCROLL_KEY);
    return isPromotionHash(stored) ? stored : "";
  } catch {
    return "";
  }
}

export function scrollToPromotionTarget(targetId, fallbackOffset = 120) {
  ...
  const raw = getComputedStyle(document.documentElement)
    ...
    .trim();
  const offset = Number.parseFloat(raw) || fallbackOffset;
  const top = window.scrollY + el.getBoundingClientRect().top - offset;
  window.scrollTo({ top: Math.max(0, top), behavior: "smooth" });
  return true;
}

/** Keep trying until the section exists (dashboard/guest banners load async). */
export function waitAndScrollToPromotion(targetId, fallbackOffset = 120) {
  if (typeof window === "undefined") return () => {};

  let tries = 0;
  let timer = 0;
  const tick = () => {
    if (scrollToPromotionTarget(targetId, fallbackOffset)) return;
    tries += 1;
    if (tries < 50) timer = window.setTimeout(tick, 100);
  };
  tick();
