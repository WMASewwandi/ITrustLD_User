...
export function getMembershipProgress(points, tierName = null) {
  const pts = Number(points) || 0;
  const current = tierName
    ? MEMBERSHIP_TIERS.find((t) => t.name.toLowerCase() === String(tierName).toLowerCase()) ||
      getMembershipTierByPoints(pts)
    : getMembershipTierByPoints(pts);
  const currentIndex = MEMBERSHIP_TIERS.findIndex((t) => t.name === current.name);
  const next = MEMBERSHIP_TIERS[currentIndex + 1] || null;
