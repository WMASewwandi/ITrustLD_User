...
export async function fetchLatestUpdates({ userType = 'normal', promotionalBanners = null } = {}) {
  const [banners, blogPosts] = await Promise.all([
    resolvePromotionalBanners(userType, promotionalBanners),
    fetchPublishedBlogPosts().catch(() => []),
  ]);

  const merged = sortLatestUpdateItems([
    ...banners.map((banner) => mapBannerToUpdateItem(banner)),
    ...blogPosts.map((post) => mapBlogToUpdateItem(post)),
  ]);

  return merged;
}

async function resolvePromotionalBanners(userType, promotionalBanners) {
  if (Array.isArray(promotionalBanners) && promotionalBanners.length > 0) {
    return promotionalBanners;
  }

  const audiences =
    userType === 'partner' ? ['affiliate', 'both'] : ['normal', 'both'];

  const results = await Promise.all(
    audiences.map((audience) =>
      fetchActivePromotionalBanners({ audience, displayType: 'all' }).catch(() => []),
    ),
  );

  const seen = new Set();
  return results.flat().filter((banner) => {
