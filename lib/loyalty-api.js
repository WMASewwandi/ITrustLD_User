import { apiRequest } from '@/lib/api';
import { getUserToken } from '@/lib/auth';
import { paymentOptionNameToAccountType } from '@/lib/payment-accounts';

function withToken(options = {}) {
  return { ...options, token: getUserToken() };
}

const LOYALTY_SUMMARY_TTL_MS = 30_000;
let loyaltySummaryCache = { data: null, at: 0, promise: null, token: null };

export function invalidateLoyaltySummaryCache() {
  loyaltySummaryCache = { data: null, at: 0, promise: null, token: null };
}

export async function fetchLoyaltySummary({ force = false } = {}) {
  const token = getUserToken();
  if (loyaltySummaryCache.token !== token) {
    loyaltySummaryCache = { data: null, at: 0, promise: null, token };
  }

  const now = Date.now();
  if (!force && loyaltySummaryCache.data && now - loyaltySummaryCache.at < LOYALTY_SUMMARY_TTL_MS) {
    return loyaltySummaryCache.data;
  }
  if (!force && loyaltySummaryCache.promise) {
    return loyaltySummaryCache.promise;
  }

  const request = apiRequest('/user/loyalty/summary', withToken())
    .then((data) => {
      loyaltySummaryCache = { data, at: Date.now(), promise: null, token };
      return data;
    })
    .catch((error) => {
      loyaltySummaryCache.promise = null;
      throw error;
    });

  loyaltySummaryCache.promise = request;
  return request;
}

export async function fetchPartnerClients(params = {}) {
  const search = new URLSearchParams();
  if (params.page) search.set('page', String(params.page));
  if (params.perPage) search.set('per_page', String(params.perPage));
  if (params.search) search.set('keyword', params.search);
  if (params.countOnly) search.set('count_only', '1');
  const qs = search.toString();
  return apiRequest(`/user/loyalty/clients${qs ? `?${qs}` : ''}`, withToken());
}

export async function fetchSubPartnerClients(params = {}) {
  const search = new URLSearchParams();
  if (params.page) search.set('page', String(params.page));
  if (params.perPage) search.set('per_page', String(params.perPage));
  if (params.search) search.set('keyword', params.search);
  if (params.countOnly) search.set('count_only', '1');
  const qs = search.toString();
  return apiRequest(`/user/loyalty/sub-partners${qs ? `?${qs}` : ''}`, withToken());
}

export function mapAffiliateClientRows(rows = []) {
  return rows.map((row) => ({
    id: row.id,
    accountId: row.accountId || row.account_id,
    isPartner: Boolean(row.isPartner ?? row.is_partner),
    firstTransaction: row.firstTransaction || row.first_transaction || '—',
    lastTransaction: row.lastTransaction || row.last_transaction || '—',
    points: row.points || '0.00',
    pointsRaw: Number(row.points_raw ?? row.points ?? 0),
  }));
}

export async function fetchLoyaltyWithdrawals(params = {}) {
  const search = new URLSearchParams();
  if (params.page) search.set('page', String(params.page));
  if (params.perPage) search.set('per_page', String(params.perPage));
  if (params.status && params.status !== 'All Statuses') search.set('status', params.status);
  if (params.search) search.set('search', params.search);
  if (params.from) search.set('from_date', params.from);
  if (params.to) search.set('to_date', params.to);
  const qs = search.toString();
  return apiRequest(`/user/loyalty/withdrawals${qs ? `?${qs}` : ''}`, withToken());
}

export async function createLoyaltyWithdrawal({
  withdrawal_point_amount,
  selected_account_id,
  selected_account_type,
}) {
  return apiRequest(
    '/user/loyalty/withdrawals',
    withToken({
      method: 'POST',
      body: {
        withdrawal_point_amount,
        selected_account_id,
        selected_account_type,
      },
    }),
  );
}

export function encodeReceivingAccountOption(account, paymentOption) {
  const accountType = paymentOptionNameToAccountType(paymentOption || account.accountType);
  return `${accountType}:${account.id}`;
}

export function decodeReceivingAccountOption(value) {
  const [accountType, accountId] = String(value || '').split(':');
  return { accountType, accountId };
}

export function flattenAccountGroups(accountGroups = [], allowedTypes = null) {
  const allowed = Array.isArray(allowedTypes)
    ? allowedTypes.map((type) => paymentOptionNameToAccountType(type))
    : null;
  const options = [];
  for (const group of accountGroups) {
    const paymentOption = group.payment_option;
    const rawMin = group.min_limit ?? group.minLimit;
    const rawMax = group.max_limit ?? group.maxLimit;
    const minLimit = rawMin == null || rawMin === '' ? NaN : Number(rawMin);
    const maxLimit = rawMax == null || rawMax === '' ? NaN : Number(rawMax);
    const currency = String(group.currency || 'USD').trim().toUpperCase() || 'USD';
    const conversionRate = Number(group.conversion_rate ?? group.conversionRate ?? 1);
    for (const account of group.accounts || []) {
      const accountType = paymentOptionNameToAccountType(paymentOption || account.accountType);
      if (allowed && !allowed.includes(accountType)) continue;
      options.push({
        value: encodeReceivingAccountOption(account, paymentOption),
        label: account.display || `${paymentOption} — ${account.id}`,
        accountId: account.id,
        accountType,
        minLimit: Number.isFinite(minLimit) ? minLimit : null,
        maxLimit: Number.isFinite(maxLimit) ? maxLimit : null,
        currency,
        conversionRate: Number.isFinite(conversionRate) && conversionRate > 0 ? conversionRate : 1,
      });
    }
  }
  return options;
}

function formatPaymentLimit(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return String(value ?? '');
  return n.toLocaleString('en-US', {
    minimumFractionDigits: 0,
    maximumFractionDigits: 2,
  });
}

export function pointsToCashoutUsd(points, pointDivider, usdPerBlock) {
  const amount = Number(points);
  const divider = Number(pointDivider);
  const usd = Number(usdPerBlock);
  if (!Number.isFinite(amount) || amount <= 0 || !Number.isFinite(divider) || divider <= 0) {
    return 0;
  }
  return (amount / divider) * (Number.isFinite(usd) ? usd : 0);
}

export function getLoyaltyCashoutLimitError({
  points,
  accountOption,
  pointDivider,
  usdPerBlock,
}) {
  if (!accountOption) return '';
  const min = Number(accountOption.minLimit);
  const max = Number(accountOption.maxLimit);
  if (!Number.isFinite(min) || !Number.isFinite(max) || (min === 0 && max === 0)) return '';

  const usd = pointsToCashoutUsd(points, pointDivider, usdPerBlock);
  if (usd <= 0) return '';

  const currency = String(accountOption.currency || 'USD').trim().toUpperCase() || 'USD';
  const amount = currency === 'LKR' ? usd * Number(accountOption.conversionRate || 1) : usd;
  const rounded = Math.round((amount + Number.EPSILON) * 100) / 100;

  if (rounded < min || rounded > max) {
    return `Cash-out amount must be between ${currency} ${formatPaymentLimit(min)} and ${currency} ${formatPaymentLimit(max)}.`;
  }
  return '';
}

export function mapWithdrawalRows(rows = []) {
  return rows.map((row) => ({
    id: row.id,
    type: row.type || 'Loyalty Cash-out',
    method: row.method || 'Point Withdrawal',
    points: row.points_display || Number(row.points || 0).toLocaleString(),
    amount: row.amount || `USD ${Number(row.cashout_amount || 0).toFixed(2)}`,
    receivingAmount: row.receiving_amount || row.received_amount || '—',
    currency: row.currency || 'USD',
    paymentOption: row.payment_option || '—',
    account: row.account || '—',
    date: row.date || row.datetime?.slice(0, 10) || '',
    time: row.time || (row.datetime ? String(row.datetime).slice(11, 16) : ''),
    status: row.status || 'Pending',
  }));
}

export async function fetchBonusClaims(params = {}) {
  const search = new URLSearchParams();
  if (params.page) search.set('page', String(params.page));
  if (params.perPage) search.set('per_page', String(params.perPage));
  if (params.status && params.status !== 'All Statuses') search.set('status', params.status);
  if (params.search) search.set('search', params.search);
  if (params.from) search.set('from_date', params.from);
  if (params.to) search.set('to_date', params.to);
  const qs = search.toString();
  return apiRequest(`/user/loyalty/bonus-claims${qs ? `?${qs}` : ''}`, withToken());
}

export async function createBonusClaim({ selected_account_id, selected_account_type }) {
  return apiRequest(
    '/user/loyalty/bonus-claims',
    withToken({
      method: 'POST',
      body: {
        selected_account_id,
        selected_account_type,
      },
    }),
  );
}

export function mapBonusClaimRows(rows = []) {
  return rows.map((row) => ({
    id: row.id,
    date: row.date || '',
    amount: row.amount || '0.00',
    method: row.method || '—',
    received: row.received || '—',
    status: row.status || 'Pending',
    rejectReason: row.rejectReason || row.rejection_reason || '',
  }));
}

export async function fetchVoucherTopupMethods() {
  return apiRequest('/user/loyalty/topup-methods', withToken());
}

export async function fetchVoucherClaims(params = {}) {
  const search = new URLSearchParams();
  if (params.page) search.set('page', String(params.page));
  if (params.perPage) search.set('per_page', String(params.perPage));
  const qs = search.toString();
  return apiRequest(`/user/loyalty/vouchers${qs ? `?${qs}` : ''}`, withToken());
}

export async function createClientBonusVoucher({ topup_method_id, platform_id }) {
  return apiRequest(
    '/user/loyalty/vouchers',
    withToken({
      method: 'POST',
      body: {
        topup_method_id,
        platform_id,
      },
    }),
  );
}

export async function fetchVoucherByToken(token) {
  return apiRequest(`/user/loyalty/vouchers/${encodeURIComponent(token)}`, withToken());
}

export async function fetchAvailableGifts() {
  return apiRequest('/user/loyalty/gifts', withToken());
}

export async function claimGift({ giftId, deliveryAddress, contactPhone }) {
  return apiRequest(
    '/user/loyalty/gifts/claim',
    withToken({
      method: 'POST',
      body: {
        gift_id: giftId,
        delivery_address: deliveryAddress,
        contact_phone: contactPhone,
      },
    }),
  );
}

export async function fetchGiftClaims() {
  return apiRequest('/user/loyalty/gift-claims', withToken());
}

export function mapGiftClaimRows(rows = []) {
  return rows.map((row) => ({
    id: row.id,
    giftId: row.gift_id,
    giftTitle: row.gift_title || '—',
    deliveryAddress: row.delivery_address || '',
    contactPhone: row.contact_phone || '',
    status: row.status || 'Pending',
    rejectionReason: row.rejection_reason || '',
    date: row.date || row.created_at || '—',
    processedAt: row.processed_at || '',
  }));
}

export function mapVoucherClaimRows(rows = []) {
  return rows.map((row) => ({
    id: row.id,
    token: row.token || '—',
    platformId: row.platform_id || '—',
    amount: row.amount_display || `USD ${Number(row.amount || 0).toFixed(2)}`,
    topupMethod: row.topup_method || row.method || '—',
    status: row.status || 'Pending',
    isClaimed: Boolean(row.is_claimed),
    createdAt: row.created_at || '—',
    expiresAt: row.expires_at || null,
    validUntil: row.valid_until || null,
    tier: row.tier || null,
    tierLabel: row.tier_label || null,
    voucherUrl: row.voucher_url || null,
    rejectReason: row.rejectReason || row.rejection_reason || '',
  }));
}

export function getActionableClaimsCount(summary = {}) {
  const bonusCount = summary?.bonus_summary?.available ? 1 : 0;
  const voucherSlots = Number(summary?.client_bonus_summary?.remaining_slots || 0);
  const giftCount = Number(summary?.eligible_gift_count || 0);
  return bonusCount + voucherSlots + giftCount;
}

export const CLAIMS_UPDATED_EVENT = "itrustld-claims-updated";

export function notifyClaimsUpdated() {
  invalidateLoyaltySummaryCache();
  if (typeof window === "undefined") return;
  window.dispatchEvent(new Event(CLAIMS_UPDATED_EVENT));
}
