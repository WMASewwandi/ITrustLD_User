...
import { syncKycCookie } from '@/lib/kyc-access';

const TOKEN_KEY = 'itrustld_user_token';
const USER_KEY = 'itrustld_user';

export const AUTH_SESSION_CHANGED_EVENT = 'itrustld-auth-changed';
const WEBVIEW_USERNAME_KEY = 'username';

export function resolveWebViewUsername(user) {
  return String(user?.email || user?.username || user?.name || '').trim();
}

/** Same handshake Laravel used for the Android WebView saved-login probe. */
export function publishWebViewAuthChannel(user = getUserSession()) {
  if (typeof window === 'undefined') return;
  const username = resolveWebViewUsername(user);
  if (!username) return;

  let meta = document.querySelector('meta[name="current-username"]');
  if (!meta) {
    meta = document.createElement('meta');
    meta.setAttribute('name', 'current-username');
    document.head.appendChild(meta);
  }
  meta.setAttribute('content', username);

  if (window.AuthChannel && typeof window.AuthChannel.postMessage === 'function') {
    try {
      window.AuthChannel.postMessage(JSON.stringify({ username }));
    } catch {
      // WebView bridge is optional outside the Android app.
    }
  }

  try {
    window.localStorage.setItem(WEBVIEW_USERNAME_KEY, username);
  } catch {
...
export function setUserSession({ token, user }) {
  ...
  syncKycCookie(user);
  publishWebViewAuthChannel(user);
...
export function updateUserSession(user) {
  ...
  setUserSession({ token, user: { ...current, ...(user || {}) } });
}
...
export { isUnverifiedAllowedPath } from '@/lib/kyc-access';

export function resolveLoggedInPath(user, requestedPath) {
  if (isUserBanned(user)) return '/banned';
  if (userNeedsVerification(user)) return '/verify';
  const requested = String(requestedPath || '').trim();
  if (!requested || requested.startsWith('/verify')) return '/dashboard';
