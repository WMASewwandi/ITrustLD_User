...
export function claimVoucherById(state, id) {
  const now = new Date();
  ...
  const vouchers = state.vouchers.map((v) =>
    v.id === id ? { ...v, userStatus: "Claimed", claimedAt: stamped } : v
  );
  const history = [
    {
      id: `h-${id}-${Date.now()}`,
...
export function claimBonusById(state, id) {
  const now = new Date();
  const stamped = `${now.toISOString().slice(0, 10)} ${now.toTimeString().slice(0, 5)}`;
  ...
  const bonuses = state.bonuses.map((b) =>
    b.id === id ? { ...b, userStatus: "Claimed", claimedAt: stamped } : b
  );
  const history = [
    {
      id: `h-${id}-${Date.now()}`,
