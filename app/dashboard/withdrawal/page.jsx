...
const METHODS = [
  { id: "xm", label: "XM", subtitle: "Local withdrawal", min: 5, max: 20000, accent: "#E11D48", icon: "xm" },
  { id: "usdt", label: "USDT", subtitle: "Tether crypto", min: 10, max: 20000, accent: "#26A17B", icon: "usdt" },
  { id: "skrill", label: "Skrill", subtitle: "E-wallet", min: 100, max: 20000, accent: "#8B2BE2", icon: "skrill" },
  { id: "neteller", label: "Neteller", subtitle: "E-wallet", min: 100, max: 20000, accent: "#00A651", icon: "neteller" },
  { id: "pm", label: "Perfect Money", subtitle: "U + 8 digits", min: 100, max: 20000, accent: "#D4AF37", icon: "pm" },
  { id: "binance", label: "Binance", subtitle: "BTC & crypto", min: 10, max: 20000, accent: "#F0B90B", icon: "btc" },
  { id: "bank", label: "Bank Transfer", subtitle: "Local banks", min: 5, max: 20000, accent: "#3B82F6", icon: "bank" },
];
...
export default function WithdrawalPage() {
  ...
  const fieldClass =
    "w-full rounded-xl border border-white/12 bg-[#0B1020]/60 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/30 focus:border-rose-400/40";
  ...
  return (
    <div className="mx-auto w-full max-w-[1100px] px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-8">
        <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#FB7185]">Payouts</p>
        <h1 className="text-3xl font-bold text-white sm:text-4xl">Withdrawal</h1>
        <p className="mt-2 text-sm text-white/50 sm:text-base">
          Withdraw funds to your bank, wallet, or payment platform. Loyalty point redemptions are on the Loyalty page.
        </p>
        <Link href="/dashboard/loyalty" className="mt-3 inline-block text-sm text-theme-green-action hover:underline">
          Go to Loyalty Withdraw →
        </Link>
      </div>

      <form onSubmit={onSubmit} className="space-y-6">
        <section className="rounded-2xl border border-white/12 bg-[#0B1020]/85 p-5 shadow-[0_20px_50px_rgba(0,0,0,0.35)] sm:p-7">
          <label className="mb-3 block text-sm font-semibold text-white">Withdrawal Amount</label>
          <div className="flex flex-col gap-3 sm:flex-row">
            <input
              ...
              className={`${fieldClass} flex-1 ${errors.amount ? "border-rose-400/50" : ""}`}
        ...
        <section className="rounded-2xl border border-white/12 bg-[#0B1020]/85 p-5 shadow-[0_20px_50px_rgba(0,0,0,0.35)] sm:p-7">
          <h2 className="text-lg font-semibold text-white">Withdrawal Method</h2>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {METHODS.map((m) => {
              ...
              return (
                ...
                >
                  {active ? (
                    <span className="absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full bg-[#E11D48] text-white">
        ...
        {method ? (
          <section className="rounded-2xl border border-white/12 bg-[#0B1020]/85 p-5 shadow-[0_20px_50px_rgba(0,0,0,0.35)] sm:p-7">
            ...
            <div className="space-y-4">
              ...
              {["skrill", "neteller", "binance", "usdt"].includes(method.id) ? (
                <div>
                  <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-white/45">
                    {method.id === "binance" ? "Binance Email ID *" : `${method.label} Email / ID *`}
                  </label>
                  <input
                    type="email"
              ...
              {method.id === "pm" ? (
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-white/35">Pre-typed U + remaining 8 digits · min USD 100</p>
              ...
              {method.id === "bank" ? (
                <div>
                  ...
                  <Link href="/dashboard/profile" className="mt-2 inline-block text-xs text-[#FB7185] hover:underline">
                    Manage saved banks
                  </Link>
                </div>
              ) : null}
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <button
                type="submit"
                className="inline-flex items-center gap-2 rounded-xl bg-[#E11D48] px-7 py-3 text-sm font-semibold text-white shadow-[0_12px_28px_rgba(225,29,72,0.3)] transition hover:brightness-110"
              >
                Submit withdrawal
          ...
          <p className="rounded-2xl border border-dashed border-white/15 bg-white/[0.03] px-5 py-6 text-center text-sm text-white/45">
            Select a withdrawal method above to continue.
          </p>
        )}
      </form>
    </div>
